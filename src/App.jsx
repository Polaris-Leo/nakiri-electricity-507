...
import { Moon, Sun, Zap, Activity, RefreshCw, TrendingDown, TrendingUp, BatteryCharging, Clock, CalendarClock, CalendarDays, History, Home, CloudLightning } from 'lucide-react';
...
export default function App() {
  ...
  // 移除 scraping 状态，因为我们将直接跳转页面，不需要在当前页维护 loading 状态
  ...
  const fetchData = useCallback(async (isInitial = false) => {
      ...
      try {
          // 1. Get Config (仅初始化时获取一次即可，但为了简单这里每次都拿也没事)
  ...
  const handleManualScrape = () => {
      // 直接跳转到 /api/scrape，由 Worker 返回美化的 HTML 结果页面
      // 浏览器会发起 GET 请求，不带 application/json 头，因此 Worker 会返回 HTML
      window.location.href = '/api/scrape';
  };
